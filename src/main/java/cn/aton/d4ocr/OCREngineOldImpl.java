...
class OCREngineOldImpl implements OCREngine {
    ...
    public String recognize(BufferedImage image) {
        try {
            ...
            // 获取ONNX会话并运行推理
            OrtSession session = ONNXRuntimeUtils.createSession(modelFile.getAbsolutePath());
            OnnxTensor inputTensor = OnnxTensor.createTensor(ONNXRuntimeUtils.getEnvironment(), FloatBuffer.wrap(data),
                    shape);
            OnnxTensor indexTensor = (OnnxTensor) session.run(Map.of("input1", inputTensor)).get(0);

            LogUtils.printMessage("score type: " + indexTensor.getInfo().type.name(), LogUtils.Level.DEBUG);
            LogUtils.printMessage("score shape: " + Arrays.toString(indexTensor.getInfo().getShape()),
